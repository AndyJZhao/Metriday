...
- Activity presentation follows RescueTime semantics: the first column is App with a light-gray icon tile, the second column is Category, and category owns the visual color. Focused categories use a deep blue; Distracting categories use red. App, website, and Item identity must not override category color; category rules expose those three sources separately.
