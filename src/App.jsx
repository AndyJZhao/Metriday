...
function TimelineContextMenu({ title, subtitle, actionLabel, actionIcon: ActionIcon = Clock, x, y, onClose, onAction }) {
  useEffect(() => {
    const closeOnOutsidePointer = (event) => {
      if (!event.target.closest?.(".activity-context-menu")) onClose();
    };
...
function WebActivityTimeline({ activities, dateKey, api, onSelect, onEditTimeEntry, onRecordCalendarEvent, onCreateTimeEntry, orientation: requestedOrientation = "vertical", onToggleOrientation }) {
 ...
 const totalSeconds = 24 * 60 * 60;
  ...
  const rangeStyle = (startSecond, endSecond, color) => {
    const startPercent = (startSecond / totalSeconds) * 100;
    const durationPercent = Math.max((endSecond - startSecond) / totalSeconds * 100, 0.4);
...
function StatsDonut({ rows, total, label }) {
  ...
  return <div className="stats-donut" role="img" aria-label={label} style={{ background: stops.length ? `conic-gradient(${stops.join(",")})` : "#eff0f3" }}><span>{formatDurationSeconds(total)}</span></div>;
}
