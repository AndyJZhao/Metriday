...
function WebTimeEntryDialog({ mode, open, api, projects, recentEntries, dateKey, initialEntry = null, onClose }) {
  ...
  const titleSuggestions = titleQuery.startsWith("$") ? [] : [...recentEntries
    .slice()
    ...
    .filter((value) => value && (!titleQuery || value.toLowerCase().includes(titleQuery)))
    .filter((value, index, values) => values.findIndex((candidate) => candidate.toLowerCase() === value.toLowerCase()) === index)
    .slice(0, 6);
  const billingShortcuts = titleQuery.startsWith("$")
    ? [
      ["billable", "Billable"],
